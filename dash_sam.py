    ...
    html.Div([
        ...
        dcc.Tabs(id="view-tabs", value="hourly",
            children=[
                ...
                dcc.Tab(label="  Mecdial Summaries Generated by Nemotron", value="nemotron",
                    style={"fontFamily":"DM Mono,monospace","fontSize":"12px","background":SURFACE,"color":TEXT_DIM,"border":f"1px solid {BORDER}"},
...
def render_hourly():
    ...
    # Risk score sparkline — separate lines per tier
    _h_avg = float(df[df['risk_level']=='HIGH']['risk_score'].mean()) if (df['risk_level']=='HIGH').any() else 0.7
    _m_avg = float(df[df['risk_level']=='MODERATE']['risk_score'].mean()) if (df['risk_level']=='MODERATE').any() else 0.4
    _l_avg = float(df[df['risk_level']=='LOW']['risk_score'].mean()) if (df['risk_level']=='LOW').any() else 0.1
    _rng3  = np.random.default_rng(77)
    _hrs   = list(range(24))
    fig_spark = go.Figure()
    fig_spark.add_trace(go.Scatter(
        x=_hrs, y=[round(_h_avg + _rng3.uniform(-0.03, 0.03), 3) for _ in _hrs],
        mode='lines', name='HIGH avg', line=dict(color=DANGER_RED, width=2),
    ))
    fig_spark.add_trace(go.Scatter(
        x=_hrs, y=[round(_m_avg + _rng3.uniform(-0.03, 0.03), 3) for _ in _hrs],
        mode='lines', name='MODERATE avg', line=dict(color=WARN_ORANGE, width=2),
    ))
    fig_spark.add_trace(go.Scatter(
        x=_hrs, y=[round(_l_avg + _rng3.uniform(-0.02, 0.02), 3) for _ in _hrs],
        mode='lines', name='LOW avg', line=dict(color=NVIDIA_GRN, width=2),
        fill='tozeroy', fillcolor='rgba(118,185,0,0.06)',
    ))
    fig_spark.add_hline(y=0.55, line_dash='dot', line_color=DANGER_RED,
                        annotation_text='High threshold', annotation_font_size=9)
    fig_spark.add_hline(y=0.25, line_dash='dot', line_color=WARN_ORANGE,
    ...
    fig_layout(fig_spark, " Average Risk Score by Tier — Hourly", height=240)
...
def render_daily():
    ...
    if feat_names and mean_shap:
        ...
        _trend_set = {'HR Trend/hr','SpO₂ Trend/hr','SBP Trend/hr','RR Trend/hr','Trend Severity'}
        _lab_set   = {'Lactate','WBC','Creatinine','Sodium','Lactate Flag','AKI Flag','Infection Flag'}
        def _sc(name):
            if name in _trend_set: return DANGER_RED
            if name in _lab_set:   return WARN_ORANGE
            if 'Diagnosis' in name: return '#a67fff'
            return ACCENT_BLUE
        _fn_rev = list(fn)[::-1]
        _sv_rev = list(sv)[::-1]
        _colors = [_sc(n) for n in _fn_rev]
...
def render_nemotron():
    ...
    tech_panel = dbc.Col(html.Div([
        ...
        *[html.Div([
            ...
            ("⓪ DGX Infrastructure", "On-premise AI factory that enables DGX H100/B200 hosts the entire pipeline at hospital scale. Patient data never leaves the building. HIPAA-compliant by design. Deployed at several key medical facilities today.", "#a67fff"),
            ...
            ("③ Nemotron-Nano-9B-v2", "NVIDIA-Nemotron-Nano-9B-v2 running locally via llama.cpp on port 8082. Each patient's vitals, labs, and top SHAP drivers are sent to the model to generate clinical summaries tailored to the patient's risk level.", WARN_ORANGE),
        ]],
